...
pub fn handle_fullscreen_add(
    mut query: Query<(&mut Geometry, &mut OriginalGeometry, &ClientWindow, &ClientFrame), Added<Fullscreened>>,
    mut state: ResMut<PlatformState>,
    monitors: Query<&Geometry, Without<ClientWindow>>,
    root_window: Res<MainRootWindow>,
) {
    for (mut geometry, mut original_geometry, window, frame) in &mut query {
    ...
    for (client, mut geometry, original_geometry, window) in &mut query {
        ...
        *dragging = Dragging(None);
        ...
        for mut tags in &mut monitors {
            ...
            let (_, frame) = match CurrentPlatform::manage(**window, *geometry, **root_window, tag, &mut commands, &mut state) {
                Err(e) => {
            ...
            CurrentPlatform::update_bordered_client_geometry(&config, *geometry, **window, *frame, &mut state);
        }
